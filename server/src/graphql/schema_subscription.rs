...
use crate::tasks::models::{ButtonChange, ControllerChange, TriggerChange};
...
type TriggerChangeStream = Pin<Box<dyn Stream<Item=TriggerChange> + Send>>;

#[graphql_subscription(Context = Context)]
impl SubscriptionRoot {
    #[graphql(description = "Receives the button updates. \
    * Only available when the controller is connected by Bluetooth, or Bluetooth+USB")]
    ...
    #[graphql(description = "Receives the trigger updates. \
    * Only available when the controller is connected by Bluetooth, or Bluetooth+USB")]
    async fn trigger_change(context: &Context) -> TriggerChangeStream {
        let mut rx = { context.ctrl_rx.clone().lock().await.clone() };
        ...
        let stream = stream! {
            while rx.changed().await.is_ok() {
                ...
                match data {
                    ControllerChange::TriggerChange(value) => yield value,
                    _ => {}
                }
            }
        };
